...
import { EventDto, MessageDto } from '../dto/eventDto';
...
export class EventService {
  ...
  removeMember(userToRemove: EventMemberDto): Observable<void> {
    return this.http.delete<void>(`${this.url}/event/remove`, {
      body: userToRemove,
    });
  }
  ...
  getBalances(balanceData: BalanceDto): Observable<ResponseBalanceDto[]> {
    return this.http.post<ResponseBalanceDto[]>(
      `${this.url}/expense/balance`,
      balanceData
    );
  }

  sendMessage(messageData: MessageDto): Observable<void> {
    return this.http.post<void>(`${this.url}/message/send`, messageData);
  }
